...
int setStopLossForAllPositions(double stopLossPrice)
{
    ...
    for(int cnt = PositionsTotal() - 1; cnt >= 0 && !IsStopped(); cnt--)
    {
        if(PositionGetTicket(cnt))
        {
            ...
            int result = sTrade.PositionModify(ticket, stopLossPrice, PositionGetDouble(POSITION_TP)); // Preserve the current TP value
            if(result == -1)
            {
            ...
            {
                Print("SL Position for ", ticket, " modified.");
            }
        }
    }

...
//--- service keys IDs
#define KEY_ALT_PRESS      (1<<31) // Alt key press identifier
#define KEY_K              107 // Key code for 'K'
//+------------------------------------------------------------------+
                  ...
                  const string &sparam)        // Third parameter depending on the event type
{
    ...
    if(id == CHARTEVENT_KEYDOWN)
    {
        if(lparam == KEY_K)
        {   
            Print("Button 'K' was pressed");
            closeAllPositions(); // Call closeAllPositions() when Alt+K is pressed
        }
    }
}
...
void setStopLossForAllPositions(double stopLossPrice)
{
    ulong st = GetMicrosecondCount();
        ...
        if(PositionGetTicket(cnt))
        {
            ...
            if(positionType == POSITION_TYPE_BUY)
            {
                ...
                if(stopLoss >= openPrice) {
                    Print("Invalid stop loss price for BUY position. It must be below the open price.");
            ...
            else if(positionType == POSITION_TYPE_SELL)
            {
                ...
                if(stopLoss <= openPrice) {
                    Print("Invalid stop loss price for SELL position. It must be above the open price.");
                    continue; // Skip this position if the stop loss is not valid
                }
            }

            // Modify the position to set the stop loss
            if(!sTrade.PositionModify(ticket, stopLoss, 0)) // The second parameter is stopLoss, and the third is take profit (set to 0)
            {
                Print("Failed to set Stop Loss for position ", ticket, ". Error: ", sTrade.ResultRetcode());
            }
            else
            {
                Print("Stop Loss set for position ", ticket, " at ", stopLoss);
            }
        }
    }

    // Monitor the process and log the elapsed time
    for(int i = 0; i < 100; i++)
    {
        Print(IntegerToString(GetMicrosecondCount() - st) + " microseconds elapsed. Positions remaining: ", IntegerToString(PositionsTotal()));
        if(PositionsTotal() <= 0) { break; }
        Sleep(100);
    }
}
...
void setTakeProfitForAllPositions(double takeProfitPrice)
{
    ulong st = GetMicrosecondCount();
    ...
    for(int cnt = PositionsTotal() - 1; cnt >= 0 && !IsStopped(); cnt--)
    {
        if(PositionGetTicket(cnt))
        {
            ...
            double openPrice = PositionGetDouble(POSITION_PRICE_OPEN); // Get the open price of the position
            int positionType = PositionGetInteger(POSITION_TYPE); // Get position type (BUY/SELL)
            double stopLoss = PositionGetDouble(POSITION_SL); // Retrieve current Stop Loss value (optional)
            double takeProfit = takeProfitPrice; // Set the desired take profit price

            // Modify the position to set the take profit
            if(!sTrade.PositionModify(ticket, stopLoss, takeProfit)) // Correct overload with ticket, stopLoss, and takeProfit
            {
                Print("Failed to set Take Profit for position ", ticket, ". Error: ", sTrade.ResultRetcode());
            }
            else
            {
                Print("Take Profit set for position ", ticket, " at ", takeProfit);
            }
        }
    }
